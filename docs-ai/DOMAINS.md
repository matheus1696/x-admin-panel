# Domains Map

Mapa rapido dos dominios reais do sistema.

## Nucleo

- [Organization](domains/ORGANIZATION.md): organograma e workflow
- [Task](domains/TASK.md): hubs, tarefas, etapas e kanban
- [Administration](domains/ADMINISTRATION.md): usuarios, permissoes e catalogos de task
- [Configuration](domains/CONFIGURATION.md): cadastros base

## Suporte

- [Audit](domains/AUDIT.md): rastreabilidade
- [Auth](domains/AUTH.md): login, registro e recuperacao
- [Profile](domains/PROFILE.md): autoatendimento do usuario autenticado
- [Public](domains/PUBLIC.md): superficie publica de contato
- [Dashboard](domains/DASHBOARD.md): entrada autenticada

## Guias De Expansao

- [Assets Prompt Guide](module/assets.md)
- [TimeClock Prompt Guide](module/time_clock.md)

## Escolha Rapida

- Hierarquia ou workflow: `Organization`
- Execucao operacional e kanban: `Task`
- Usuarios, permissoes e status: `Administration`
- Cadastros base: `Configuration`
- Autenticacao: `Auth`
- Perfil: `Profile`
- Logs: `Audit`
