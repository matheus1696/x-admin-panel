<div>
    <x-button
        wire:click="open"
        :text="__('assets.operations.transfer.action')"
    ...
    <x-modal :show="$showModal" wire:key="transfer-asset-modal">
        @if ($modalKey === 'transfer-asset')
            <x-slot name="header">
                <h2 class="text-sm font-semibold text-gray-700 uppercase">{{ __('assets.operations.transfer.title') }}</h2>
            </x-slot>
            ...
            <form wire:submit.prevent="save" class="grid grid-cols-1 gap-4 md:grid-cols-12">
                ...
                <div class="md:col-span-12 flex justify-end gap-2">
                    ...
                    <x-button type="submit" :text="__('assets.operations.transfer.submit')" variant="blue_solid" />
                </div>
            </form>
        @endif
    </x-modal>
