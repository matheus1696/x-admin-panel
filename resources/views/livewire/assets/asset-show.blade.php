<div>
    ...
    <x-page.header
        :title="__('assets.asset_show.title', ['code' => $asset->code])"
        :subtitle="__('assets.asset_show.subtitle')"
        ...
        <div class="xl:col-span-4">
            <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <h3 class="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
                    {{ __('assets.asset_show.summary_title') }}
                </h3>

                <dl class="space-y-3 text-sm">
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.description') }}</dt>
                        <dd class="text-gray-700">{{ $asset->description }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.state') }}</dt>
                        <dd class="text-gray-700">{{ __('assets.states.'.strtolower($asset->state->value)) }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.unit') }}</dt>
                        <dd class="text-gray-700">{{ $asset->unit?->title ?? '-' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.sector') }}</dt>
                        <dd class="text-gray-700">{{ $asset->sector?->title ?? '-' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.serial_number') }}</dt>
                        <dd class="text-gray-700">{{ $asset->serial_number ?: '-' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.patrimony_number') }}</dt>
                        <dd class="text-gray-700">{{ $asset->patrimony_number ?: '-' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase text-gray-400">{{ __('assets.asset_show.fields.invoice') }}</dt>
                ...
                <div class="mb-4 flex items-center justify-between gap-3">
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-500">
                            {{ __('assets.asset_show.timeline_title') }}
                        </h3>
                        <p class="mt-1 text-xs text-gray-500">
                            {{ __('assets.asset_show.timeline_count', ['count' => $totalEvents]) }}
                        </p>
                    </div>
                </div>
                            ...
                            <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                <div>
                                    <p class="text-sm font-semibold text-gray-800">
                                        {{ __('assets.events.'.$event->type->value) }}
                                    </p>
                                    <p class="mt-1 text-xs text-gray-500">
                                        {{ $event->created_at?->format('d/m/Y H:i') }} · {{ $event->actor?->name ?? __('assets.asset_show.labels.system') }}
                                    </p>
                                </div>

                                <div class="text-xs text-gray-600">
                                    @if ($event->from_unit_id || $event->to_unit_id)
                                        <p>{{ __('assets.asset_show.labels.unit_change', ['from' => $event->fromUnit?->title ?? '-', 'to' => $event->toUnit?->title ?? '-']) }}</p>
                                    @endif
                                    @if ($event->from_sector_id || $event->to_sector_id)
                                        <p>{{ __('assets.asset_show.labels.sector_change', ['from' => $event->fromSector?->title ?? '-', 'to' => $event->toSector?->title ?? '-']) }}</p>
                                    @endif
                                    @if ($event->from_state || $event->to_state)
                                        <p>{{ __('assets.asset_show.labels.state_change', ['from' => $event->from_state ?? '-', 'to' => $event->to_state ?? '-']) }}</p>
                                    @endif
                                </div>
                            </div>
                ...
                @if ($events->count() < $totalEvents)
                    <div class="mt-4 flex justify-center">
                        <x-button
                            ...
                            :text="__('assets.asset_show.actions.load_more')"
