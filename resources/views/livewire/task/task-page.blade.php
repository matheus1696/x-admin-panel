<div>
    ...
    <div x-data="{ openAsideTask: false }">
        ...
        <div class="relative overflow-hidden border border-gray-200/80 rounded-t-2xl">
            
            <!-- Cabeçalho das Tarefas - Premium -->
            <div class="grid grid-cols-5 md:grid-cols-12 gap-4 px-6 py-4 bg-gradient-to-r from-green-600 via-green-700 to-green-800 backdrop-blur-sm border-b border-emerald-100/50 text-xs font-semibold text-white uppercase tracking-wider">
        ...
        @if ($isCreatingTask)
            <div class="py-2.5 px-2 border border-gray-200/80 ">
        ...
        <div class="divide-y divide-gray-200/50 border border-gray-200/80 rounded-b-2xl shadow-xl">
            @forelse ($tasks as $task)
                <div x-data="{ openSteps: false }" class="group/task transition-all duration-300  {{ $task->finished_at ? 'bg-gradient-to-r from-green-300/50 to-green-300/30' : 'hover:bg-gradient-to-r hover:from-green-50/50 hover:to-transparent' }}">
                    ...
                    <div class="grid grid-cols-5 md:grid-cols-12 px-6 py-3 items-center relative">
                            ...
                            <div class="flex items-center gap-1 flex-1">
                                <div class="w-5">
                                    @if ($task->taskSteps->count() > 0)
                                        <button wire:click="cancelCreateTaskStep()" @click="openSteps = !openSteps" class="w-5 h-5 flex items-center justify-center rounded-md hover:bg-emerald-100 transition-all duration-200 group/btn">
                        ...
                        <div class="col-span-2 px-2 text-center">
                            <span class="px-3 py-1 bg-white/80 rounded-full border border-gray-200/80 text-[11px] line-clamp-1">
                                {{ $task->user?->name ?? '—' }}
                            </span>
                        </div>

                        <!-- Informações Premium -->
                        <div class="col-span-5 hidden md:grid grid-cols-3 gap-2 px-2 text-[11px]">
                            
                            <!-- Categoria -->
                            <div class="flex items-center justify-center">
                                <span class="px-3 py-1 bg-white/80 rounded-full border border-gray-200/80 shadow-sm line-clamp-1">
                            ...
                            <div class="flex items-center justify-center">
                                <span class="inline-flex items-center gap-1.5 font-semibold rounded-full px-3 py-1 shadow-sm ring-1 ring-black/5
                        ...
                        <div class="col-span-1 hidden md:block">
                            <div class="flex flex-col items-center">
                                @if ($task->started_at)
                                    <span class="text-[11px] font-medium text-gray-700 bg-white/80 px-2 py-1 rounded-lg border border-gray-200/80">
                                        {{ $task->started_at->format('d/m') }}
                                    </span>
                                @else
                                    <span class="text-[11px] text-gray-400 italic bg-gray-100/50 px-2 py-1 rounded-lg">—</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="col-span-1 hidden md:block">
                            <div class="flex flex-col items-center">
                                @if ($task->deadline_at)
                                    ...
                                    @else
                                        <span class="text-[11px] font-medium px-2 py-1 rounded-lg flex items-center gap-1
                                            ...
                                            <i class="fas {{ $task->deadline_at->isPast() ? 'fa-exclamation-triangle' : 'far fa-calendar-check' }} text-[8px]"></i>
                                            {{ $task->deadline_at->format('d/m') }}
                                        </span>
                                    @endif
                                @else
                    ...
                    <div x-show="openSteps" x-collapse class="bg-gradient-to-br from-gray-50/50 via-white to-gray-50/30">
                        
                        <!-- Cabeçalho das Etapas - Premium -->
                        <div class="grid grid-cols-5 md:grid-cols-12 items-center gap-4 px-6 py-2.5 bg-gradient-to-r from-amber-50/50 via-white/80 to-amber-50/50 border-y border-amber-500/50">
                            <div class="col-span-3 flex items-center gap-2">
                        ...
                        @if (!$task->finished_at)
                            ...
                            @if ($isCreatingTaskStep && $taskId == $task->id)
                                <div class="border-b border-amber-500/500 bg-gradient-to-br from-amber-50/30 via-white to-amber-50/30 px-2 py-2.5">
                                    <div class="flex items-center gap-2">
                                        <div class="w-2.5 h-5 bg-gradient-to-b from-amber-500 to-orange-500 rounded-full"></div>
                                        <div class="flex-1">
                        ...
                        <div class="divide-y divide-amber-100/50">
                            @foreach ($task->taskSteps as $step)
                                ...
                                <div class="grid grid-cols-5 md:grid-cols-12 px-4 py-2 items-center hover:bg-amber-50/30 transition-colors duration-150 group/step">
                                    
                                    <!-- Título da Etapa -->
                                    <div class="col-span-3 flex items-center gap-2">       
                                        <button @click="openAsideTask = true;" wire:click="openAsideTaskStep({{$step->id}})" class="group/code inline-flex items-center gap-1 py-1 rounded-lg hover:bg-amber-100/50 transition-all">
                                    ...
                                    <!-- Setor -->
                                    <div class="col-span-2 text-center" title="{{ $step->organization?->title ?? '—' }}">
                                        <span class="text-xs px-2 py-1 bg-white/80 rounded-full border border-gray-200/80 inline-flex items-center gap-1">
                                            ...
                                            <span class="line-clamp-1">{{ $step->organization?->acronym ?? '—' }}</span>
                                        </span>
                                    </div>
