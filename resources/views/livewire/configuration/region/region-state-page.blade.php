<div>
    ...
    <x-page.filter title="Filtros de Estados">
        {{-- Filtros Básicos --}}
        <x-slot name="showBasic">
            <div class="md:col-span-6">
                <x-form.label value="Estado" />
                <x-form.input type="text" placeholder="Buscar por estado..." wire:model.live.debounce.500ms="name"/>
            </div>
    ...
    <x-page.table :pagination="$states">
        <x-slot name="thead">
            <tr>
                <x-page.table-th value="Estados" />
        ...
        <x-slot name="tbody">
            @foreach ($states as $state)
                <tr>
                    <x-page.table-td :value="$state->title" />
                    <x-page.table-td class="text-center">
                        <div class="text-xs font-medium rounded-full py-0.5 px-1 {{ $state->is_active ? 'bg-green-300 text-green-700' : 'bg-red-300 text-red-700' }}">
                            {{ $state->is_active ? 'Ativo' : 'Desativado' }}
                        </div>
                    </x-page.table-td>
                </tr>
            @endforeach
