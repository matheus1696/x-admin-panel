<div>
            ...
            <x-slot name="tbody">
                @foreach ($taskStatuses as $taskStatus)
                    <tr>
                        ...
                        <x-page.table-td :value="$taskStatus->title" />
            ...
            <x-slot name="tbody">
                @foreach ($taskStepStatuses as $taskStepStatus)
                    <tr>
                        ...
                        <x-page.table-td :value="$taskStepStatus->title" />
    ...
    <x-modal :show="$showModal" wire:key="workflow-modal-run-status">
        @if ($modalKey === 'modal-form-create-task-status')
            <x-slot name="header">
            ...
            <form wire:submit.prevent="storeRunStatus" class="space-y-4">
                @component('livewire.administration.task._partials.task-status-form') @endcomponent
                <div class="flex justify-end gap-2 pt-4">
                    <x-button type="submit" text="Salvar" />
                </div>
            </form>
        @endif
        @if ($modalKey === 'modal-form-edit-task-status')
            <x-slot name="header">
        ...
        @endif
        @if ($modalKey === 'modal-form-create-task-step-status')
            <x-slot name="header">
        ...
        @endif
        @if ($modalKey === 'modal-form-edit-task-step-status')
            <x-slot name="header">
