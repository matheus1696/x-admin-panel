...
<div x-data="{ openDropdown: {{ $active ? 'true' : 'false' }} }" class="relative mx-2">
    ...
    >
        ...
        <i x-show="sidebarExpanded || openAside"
           ...
           class="fas fa-chevron-right text-[10px] transition-transform duration-200 {{ $active ? 'text-white' : 'text-gray-400' }}"></i>
    </button>

    <!-- Dropdown items - visível apenas quando sidebar expandida e dropdown aberto -->
    <div x-show="openDropdown && sidebarExpanded"
         x-collapse
         class="ml-6 mt-2.5 space-y-1 border-l-2 border-emerald-200/50 pl-3.5">
        {{ $slot }}
    </div>
    ...
    <div x-show="openDropdown && !sidebarExpanded"
         x-collapse
         class="ml-6 mt-2.5 space-y-1 border-l-2 border-emerald-200/50 pl-3.5 md:hidden">
        {{ $slot }}
    </div>
</div>